...
class SCS
{
    ...
    private function SYSTEM_DEFCODE($DEFCODE = 2)
    {
        ...
        if ($DEFCODE == 1) {
            ...
            $db = new Db();
            $db->Options(['crypt' => true, 'errors' => false]);
            ...
            $db->newTb('SCS_API', ['scs_data', 'scs_ip']);
            $search = $db->Select('SCS_API', ['scs_ip' => $DEFCODE_CLIENT['IP']]);
            if ($search == null) {
                /*
                 * CALL TO A API
                 * Currently in dev
    ...
    private function CHECKING_COUNTRY()
    {
        ...
        if (file_exists('config.xml')) {
            ...
            $this->utils_json_reply(['error' => 'SCS CONFIG FILE YAML']);
        }
    }
    ...
    private function LOG_REQUEST()
    {
        ...
        fgets($LogFile);
        fwrite($LogFile, ';'.base64_encode($_SERVER['REMOTE_ADDR']));
        fclose($LogFile);
    }
